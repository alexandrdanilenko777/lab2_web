...
workbook = xlrd.open_workbook('c:/data/lab2.xlsx')
#workbook = xlrd.open_workbook('c:/data/lab2_1.xlsx')
...
def stat(lst):
    ...
    v = numpy.var(lst)
    std = numpy.std(lst)
    #print ("expected value = {}".format(numpy.mean(lst)))
    #print ("variance = {}".format(numpy.var(lst)))
    #print ("standart deviation = {}".format(numpy.std(lst)))
    return ev,v,std
...
def plot_distrib_dev(dev1,dev2,dev3,name):
    ...
    d_list.sort()
    #date = [d_lst[0],d_lst[100],d_lst[132]]
    date = []
    for i in range(len(d_list)):
        date.append(d_list[i])
    #plot(date,dev1,date,dev2,date,dev3)
...
#print (interp(DT_p[0])) #SCIPY!!!! scipy.interpolate.spline
#plot_distrib_dev(DT_p[0],DT_p[1],DT_p[2],"Device distribution")
...
def find_peak(dev1):
    ...
    for i in range(len(dev1)-10):
        ...
        i=i+10
    '''
    for i in range(len(dev1) - 3):
        mx = max(dev1[i:i + 3])
        mn = min(dev1[i:i + 3])
        mxi = dev1[i:i + 3].index(mx)
        mni = dev1[i:i + 3].index(mn)
        is_peak[mxi + i] = 1
        is_peak[mni + i] = -1
        i = i + 3'''
    return is_peak
#find_peak(DT_p[0])
#plot_distrib_dev(find_peak(DT_p[0]),find_peak(DT_p[1]),find_peak(DT_p[2]),"Device distribution")
...
plot_distrib_dev(cp,cp,cp,"Device distribution")
#print (stat(cp))
